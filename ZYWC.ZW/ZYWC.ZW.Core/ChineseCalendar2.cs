...
namespace ZYWC.ZW.Core
{
    ...
    public class ChineseCalendar2
    {
        ...
        private int _cYear;
        private int _cMonth; //农历月份(1开始)
        private int _cDay; //农历当月第几天(1开始)
        private int _cHour; //农历时辰（23~1为子时）
        private bool _cWanZi;
        ...
        /// </summary>
        /// <param name="dt"></param>
        public ChineseCalendar2(DateTime dt)
        {
            int i;
            int leap;
            int temp;
            ...
            if (dt.Hour == 23 && dt.Minute > 0) //23点之后为次日的子时
            {
                //_datetime = dt.AddDays(1);
                _datetime = dt;
                _cWanZi = true;
                _cHour = 1;
            }
            ...
            {
                _datetime = dt;
                _cHour = (dt.Hour + ((dt.Minute > 0) ? 1 : 0)) / 2 + 1;
            }
            ...
            TimeSpan ts = _date - ChineseCalendar2.MinDay;//计算两天的基本差距[即1900到当天的天差]
            offset = ts.Days;
        ...
        public ChineseCalendar2(int cy, int cm, int cd, int ch, bool leapMonthFlag)
        {
        ...
        /// 农历时
        /// </summary>
        public int ChineseHour
        {
            get { return this._cHour; }
        }
        public bool ChineseWanZi
        {
            get { return this._cWanZi; }
        }
        #endregion

        #region ChineseHourString
        /// <summary>
        /// 农历时中文表示
        /// </summary>
        public string ChineseHourString
        {
            get
            {
                return zhiStr[_cHour - 1].ToString() + "时";
            }
        }
        public string ChineseWanZiString
        {
            get { return this._cWanZi?"晚":""; }
        }
        #endregion
        ...
        #region ChineseDateTimeString
        /// <summary>
        /// 取农历日期表示法：农历一九九七年正月初五
        /// </summary>
        public string ChineseDateTimeString
        {
            get
            {
                if (this._cIsLeapMonth == true)
                {
                    return "农历" + ChineseYearString + "闰" + ChineseMonthString + ChineseDayString + ChineseHourString;
                }
                else
                {
                    return "农历" + ChineseYearString + ChineseMonthString + ChineseDayString + ChineseHourString;
                }
            }
        }
        #endregion
        ...
        public string GanZhiHourString
        {
            get
            {
                ...
                //计算时辰的地支
                _hour = this._datetime.Hour;    //获得当前时间小时
                _minute = this._datetime.Minute;  //获得当前时间分钟

                //zhiHour = zhiStr[_cHour - 1].ToString();
                ...
                tmpGan = ganStr.Substring(indexGan) + ganStr.Substring(0, indexGan + 2);//凑齐12位 lance:根据日上起时表的横纵规律进行计算

                //ganHour = ganStr[((i % 10 + 1) * 2 - 1) % 10 - 1].ToString();

                return tmpGan[_cHour - 1].ToString() + zhiStr[_cHour - 1].ToString() + "时";
            }
        }
        #endregion
        ...
        /// </summary>
        public string GanZhiDateTimeString
        {
            get
            {
        ...
        public string BaziString
        {
            get
            {
                return GanZhiYearString.Substring(0, 2) + GanZhiMonthString.Substring(0, 2) + GanZhiDayString.Substring(0, 2) + GanZhiHourString.Substring(0, 2);
            }
        }

        public int[] BaziIndex
        {
            get
            {
                int[] index = new int[8];

                for (int i = 0; i < BaziString.Length; i++)
                {
                    if (i % 2 == 0) //干
                    {
                        index[i] = ganStr.IndexOf(BaziString[i]);
                    }
                    else //支
                    {
                        index[i] = zhiStr.IndexOf(BaziString[i]);
                    }
                }
